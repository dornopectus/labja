...
export default function Admin() {
  ...
  async function carregarTudo() {
    ...
    const resultados = await Promise.all([
      ...
      supabase.from('turmas').select('id,nome,curso,quantidade_estudantes,turno,bloco,bloco_novo,ano_serie,ativo').order('nome'),
  ...
  const turmas = useMemo(() => dados.turmas.filter(t => normalizar(`${t.nome} ${t.curso} ${t.bloco} ${t.bloco_novo ? 'bloco novo' : 'bloco antigo'} ${t.ano_serie}`).includes(normalizar(busca))), [dados.turmas,busca])
  ...
  function novo(tipo) {
    const padrao = {
      ...
      turma: { id:null,nome:'',curso:'',quantidade_estudantes:'',turno:'',bloco:'',bloco_novo:false,ano_serie:'',ativo:true },
  ...
  async function salvarEditor(e) {
    ...
    try {
      ...
      if (editor.tipo === 'turma') {
        const p={nome:d.nome.trim(),curso:vazio(d.curso)?null:d.curso.trim(),quantidade_estudantes:vazio(d.quantidade_estudantes)?null:Number(d.quantidade_estudantes),turno:vazio(d.turno)?null:d.turno,bloco:vazio(d.bloco)?null:d.bloco.trim(),bloco_novo:Boolean(d.bloco_novo),ano_serie:vazio(d.ano_serie)?null:d.ano_serie.trim(),ativo:Boolean(d.ativo)}
  ...
  return <Layout>
    <main className="admin-pagina">
      ...
      {carregando?<div className="admin-carregando">Carregando dados...</div>:<>
        ...
        {aba==='turmas'&&<Tabela colunas={['Turma','Curso','Alunos','Turno','Bloco','Bloco novo','Ano/Série','Ativo','Ações']}>{turmas.map(t=><tr key={t.id}><td><strong>{t.nome}</strong></td><td>{t.curso||'—'}</td><td>{vazio(t.quantidade_estudantes)?'—':t.quantidade_estudantes}</td><td>{t.turno||'—'}</td><td>{t.bloco||'—'}</td><td><Pill ativo={t.bloco_novo}/></td><td>{t.ano_serie||'—'}</td><td><Pill ativo={t.ativo}/></td><td><Acoes onEdit={()=>editar('turma',t)} onDelete={()=>excluir('turmas',t.id,`a turma ${t.nome}`)}/></td></tr>)}</Tabela>}

...
const ABAS = [
  ...
  ['horarios', 'Horários'], ['reservas', 'Reservas'],
]
...
export default function Admin() {
  ...
  const horarios = useMemo(() => dados.horarios.filter(h => normalizar(`${nomeDia(h.dia_semana)} ${h.bloco} ${h.hora_inicio} ${h.hora_fim}`).includes(normalizar(busca))), [dados.horarios,busca])
  ...
  function novo(tipo) {
    const padrao = {
      ...
      horario: { id:null,dia_semana:1,bloco:'',hora_inicio:'',hora_fim:'' },
  ...
  async function salvarEditor(e) {
    ...
    try {
      ...
      if (editor.tipo === 'horario') {
        const p={dia_semana:Number(d.dia_semana),bloco:d.bloco.trim(),hora_inicio:d.hora_inicio,hora_fim:d.hora_fim}
        if(!p.bloco||!p.hora_inicio||!p.hora_fim)throw new Error('Preencha dia, bloco e horário.')
        const q=editor.modo==='novo'?supabase.from('horarios').insert(p):supabase.from('horarios').update(p).eq('id',d.id)
        const {error}=await q;if(error)throw error
      }
  ...
  return <Layout>
    <main className="admin-pagina">
      ...
      {carregando?<div className="admin-carregando">Carregando dados...</div>:<>
        ...
        {aba!=='resumo'&&<div className="admin-toolbar"><input className="admin-busca" type="search" value={busca} onChange={e=>setBusca(e.target.value)} placeholder={`Buscar em ${ABAS.find(x=>x[0]===aba)?.[1].toLowerCase()||'dados'}...`}/><div className="admin-toolbar-acoes">{['professores','turmas','disciplinas','laboratorios','horarios'].includes(aba)&&<button className="admin-botao principal" onClick={()=>novo(aba==='professores'?'professor':aba==='turmas'?'turma':aba==='disciplinas'?'disciplina':aba==='laboratorios'?'laboratorio':'horario')}>+ Novo</button>}{aba==='laboratorios'&&<button className="admin-botao secundario" onClick={()=>novo('prioridade')}>+ Prioridade</button>}</div></div>}
        ...
        {aba==='horarios'&&<Tabela colunas={['Dia','Bloco','Início','Fim','Ações']}>{horarios.map(h=><tr key={h.id}><td>{nomeDia(h.dia_semana)}</td><td><strong>{h.bloco}</strong></td><td>{h.hora_inicio?.slice(0,5)}</td><td>{h.hora_fim?.slice(0,5)}</td><td><Acoes onEdit={()=>editar('horario',h)} onDelete={()=>excluir('horarios',h.id,`o horário ${h.bloco}`)}/></td></tr>)}</Tabela>}
...
function Resumo({dados}){
  ...
  return <>
    <section className="admin-cards"><Card titulo="Professores" valor={dados.professores.length} detalhe={`${dados.professores.filter(x=>x.ativo).length} ativos`}/><Card titulo="Turmas" valor={dados.turmas.length} detalhe={`${dados.turmas.filter(x=>x.ativo).length} ativas`}/><Card titulo="Disciplinas" valor={dados.disciplinas.length} detalhe="cadastro"/><Card titulo="Laboratórios" valor={dados.laboratorios.length} detalhe={`${dados.laboratorios.filter(x=>x.ativo).length} ativos`}/><Card titulo="Horários" valor={dados.horarios.length} detalhe="faixas cadastradas"/><Card titulo="Reservas" valor={dados.reservas.length} detalhe={`${confirmadas} confirmadas · ${pendentes} pendentes`}/></section>
...
function Editor({editor,setEditor,salvar,salvando,dados}){
  ...
  return <Modal titulo={`${titulo} ${editor.tipo==='professor'?'professor':editor.tipo==='turma'?'turma':editor.tipo==='disciplina'?'disciplina':editor.tipo==='laboratorio'?'laboratório':editor.tipo==='horario'?'horário':'prioridade'}`} onClose={()=>setEditor(null)}>
    <form onSubmit={salvar}>
      ...
      {editor.tipo==='horario'&&<div className="admin-form-grid"><Campo label="Dia"><select value={d.dia_semana} onChange={e=>set('dia_semana',Number(e.target.value))}>{DIAS.map(([id,nome])=><option key={id} value={id}>{nome}</option>)}</select></Campo><Campo label="Bloco"><input value={d.bloco} onChange={e=>set('bloco',e.target.value)} required/></Campo><Campo label="Hora início"><input type="time" value={d.hora_inicio?.slice(0,5)??''} onChange={e=>set('hora_inicio',e.target.value)} required/></Campo><Campo label="Hora fim"><input type="time" value={d.hora_fim?.slice(0,5)??''} onChange={e=>set('hora_fim',e.target.value)} required/></Campo></div>}
